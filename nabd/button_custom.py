...
class ButtonCustom(Button):  # pragma: no cover
    BUTTON_CHANNEL = 2
    DOWN_VALUE = 0
    UP_VALUE = 1
    ...
    def __init__(self, hw_model):
        ...
        self.button_channel = ButtonCustom.BUTTON_CHANNEL
    ...
    def _button_event(self, channel):
        ...
        if (
            GPIO.input(self.button_channel) == ButtonCustom.UP_VALUE and 
            ...
            with self.button_sequence_lock:
                if self.button_sequence == 5:
                    ...
                    self.button_timer = Timer(
                        ButtonCustom.TRIPLE_CLICK_TIMEOUT, self._double_click_cb
                    )
                    self.button_timer.start()
                elif self.button_sequence == 1:
                    self.button_sequence = 2
                    self.button_timer = Timer(
                        ButtonCustom.DOUBLE_CLICK_TIMEOUT, self._click_cb
                    )
                    self.button_timer.start()
        elif (
            GPIO.input(self.button_channel) == ButtonCustom.DOWN_VALUE
            ...
            with self.button_sequence_lock:
                if self.button_sequence == 0:
                    ...
                    self.button_timer = Timer(
                        ButtonCustom.HOLD_TIMEOUT, self._hold_cb
                    )
                    self.button_timer.start()
                elif self.button_sequence == 2:
                    self.button_sequence = 3
                    self.button_timer = Timer(
                        ButtonCustom.CLICK_AND_HOLD_TIMEOUT,
                    ...
                    self.button_timer = Timer(
                        ButtonCustom.TRIPLE_CLICK_TIMEOUT,
                        self._click_and_hold_cb,
                    )
                    self.button_timer.start()
